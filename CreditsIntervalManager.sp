...
{
	g_cCreditsSystem = CreateConVar("sm_ci_system", "1", "What do you want to use? 1 = Steam Group Manager | 2 = Discord Utilities (Default 1)");
...
public Action Timer_CreditsInterval(Handle timer)
{
	...
	for(int i = 1; i <= MaxClients; i++)
	{
		...
		{
			if(IsClientInGame(i) && !IsFakeClient(i) && DU_IsMember(i))
			{
				Store_SetClientCredits(i, Store_GetClientCredits(i) + iCredits);
